...
module.exports = async function handler(req, res) {
  ...
  if (!invited.includes(email.trim().toLowerCase())) {
    ...
    return res.status(200).json({ ok: true, otpToken: '' });
  }
  ...
  // Always log OTP for debugging (visible in Vercel function logs)
  console.log(`[OTP] ${email} → ${otp}`);

  // Attempt email send — non-blocking: step 2 shows regardless
  let emailError = null;
  try {
    ...
    if (!resendRes.ok) {
      ...
      emailError = body.message || body.name || 'Unknown Resend error';
  ...
  // Always return otpToken so step 2 appears
  res.json({ ok: true, otpToken, emailError });
};
